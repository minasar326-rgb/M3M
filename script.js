...
function showSection(sectionName) {
  ...
  if (sectionName === 'reports') updateWeekSelect();
}
...
function updateWeekSelect() {
  const weeks = [...new Set(attendance.map(r => r.weekNum))].sort((a,b) => b-a);
  const select = document.getElementById('report-week');
  select.innerHTML = '<option value="">جميع الأسابيع</option>' + 
    weeks.map(w => `<option value="${w}">الأسبوع ${w}</option>`).join('');
}
...
async function generateStudentReport() {
  ...
  const records = attendance.filter(r => r.studentId === student.id);
  await createPdfReport(student, records, `تقرير_${student.name.replace(/\\s/g, '_')}`);
  
  // Also generate Word
  await generateWordReport(student, records, `تقرير_${student.name.replace(/\\s/g, '_')}`);
}
...
async function generateFullReport() {
  ...
  await generateWordReportForAll(fullData, 'تقرير_كامل_الحضور');
}

async function generateWordReport(student, records, filename) {
  const { Document, Packer, Paragraph, Table, TableRow, TableCell, TextRun, Header, AlignmentType } = docx;
  
  const tableRows = records.map(r => new TableRow({
    children: [
      ...
      headers: {
        default: new Header({
          children: [new Paragraph({
            ...
            children: [new TextRun("تقرير حضور الطالب")],
...
async function generateAllReport() {
  ...
  const filteredAttendance = weekFilter 
    ? attendance.filter(r => r.weekNum == weekFilter)
    : attendance;
  
  for (const student of students) {
    const records = filteredAttendance.filter(r => r.studentId === student.id);
    await createPdfReport(student, records, `تقرير_${student.name.replace(/\\s/g, '_')}_${weekFilter || 'كامل'}`);
  }
  alert('تم تصدير جميع PDF!');
}


async function createPdfReport(student, records, filename) {
  ...
  doc.text(`تقرير حضور الطالب: ${student.name}`, 14, 25);
  
  doc.setFontSize(14);
  doc.setFont('helvetica', 'normal');
  doc.text(`كود الطالب: ${student.code}`, 14, 35);
  
  const stats = getStudentStats(student.id);
  doc.text(`الحضور: ${stats.present} | الغياب: ${stats.absent} | النسبة: ${stats.percentage}%`, 14, 45);
  
  // Table headers
  const headers = [['التاريخ', 'الوقت', 'اليوم', 'الجزء', 'الحالة']];
  const data = records.map(r => [
    r.date,
    r.time,
    r.day === 1 ? 'الخميس' : 'الجمعة',
    r.part === 1 ? 'الجزء 1' : 'الجزء 2',
    r.status === 'present' ? '✅ حاضر' : '❌ غائب'
  ]);
  
  doc.autoTable({
    head: headers,
    body: data.slice(0, 20), // Limit rows
    startY: 55,
    theme: 'grid',
    ...
    didDrawPage: function(data) {
      // Page footer
      doc.setFontSize(8);
      doc.text(`صفحة ${doc.internal.getNumberOfPages()}`, 190, 285, { align: 'right' });
    }
  });
  
  if (records.length > 20) {
    doc.setFontSize(12);
    doc.text('... و باقي السجلات', 14, doc.lastAutoTable.finalY + 10);
  }
...
async function startBarcodeFullScanner() {
  ...
  try {
    quaggaInstance = Quagga.init({
      ...
      Quagga.start();
      isScanning = true;
      console.log("Full screen scanner started");
    });
...
async function onBarcodeDetected(result) {
  ...
  // Find EXISTING student only
  const student = students.find(s => s.code.toString() === code);
  if (!student) {
    updateBarcodeStatus(`❌ لا يوجد طالب بالكود: ${code}`, 'red');
    setTimeout(() => {
      updateBarcodeStatus('اختر الجزء ثم ابدأ المسح');
      document.getElementById('start-scan-full-btn').classList.remove('hidden');
    }, 2500);
    return;
  }
